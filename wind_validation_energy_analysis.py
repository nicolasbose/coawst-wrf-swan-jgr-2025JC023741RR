...
# OWF energy-map bounding box — derived from make_bounding_box_from_grid()
# lat [−4.9500, −4.8419], lon [−36.4000, −36.0385]
OWF_LAT_MIN, OWF_LAT_MAX = -4.9500, -4.8419
OWF_LON_MIN, OWF_LON_MAX = -36.4000, -36.0385
...
def load_model_datasets(database_dir: Path) -> dict[str, xr.Dataset]:
    wrfswan_file = database_dir / "wrfswan_100m.nc"
    wrfstand_file = database_dir / "wrfstand_100m.nc"
    wrfswan_energy_file = database_dir / "energy_10MW_wrfswan.nc"
    wrfstand_energy_file = database_dir / "energy_10MW_wrfstand.nc"
    assert_required_files([wrfswan_file, wrfstand_file, wrfswan_energy_file, wrfstand_energy_file])
    return {
        "wrfswan": xr.open_dataset(wrfswan_file)[["wspd", "wdir"]],
        "wrfstand": xr.open_dataset(wrfstand_file)[["wspd", "wdir"]],
        "wrfswan_energy": xr.open_dataset(wrfswan_energy_file).rename({"power_15m": "power_10m"}),
        "wrfstand_energy": xr.open_dataset(wrfstand_energy_file).rename({"power_15m": "power_10m"}),
    }
...
def build_energy_summary(
    ...
    owf_swan = wrfswan_energy_ds.sel(
        y=slice(OWF_LAT_MIN, OWF_LAT_MAX), x=slice(OWF_LON_MIN, OWF_LON_MAX)
    )
    owf_stand = wrfstand_energy_ds.sel(
        y=slice(OWF_LAT_MIN, OWF_LAT_MAX), x=slice(OWF_LON_MIN, OWF_LON_MAX)
    )
    owf_diff = energy_difference.sel(
        y=slice(OWF_LAT_MIN, OWF_LAT_MAX), x=slice(OWF_LON_MIN, OWF_LON_MAX)
    )
    swan_total = float((owf_swan[power_var].sum(dim="time") / 1000.0).sum().values)
    stand_total = float((owf_stand[power_var].sum(dim="time") / 1000.0).sum().values)
    diff_total = float(owf_diff.sum().values)
    rel_diff = 100.0 * diff_total / swan_total if swan_total != 0 else float("nan")
    return pd.DataFrame(
        {
            "WRF-SWAN total energy [MWh]": [swan_total],
            "WRF total energy [MWh]": [stand_total],
            "Difference [MWh]": [diff_total],
            "Relative difference [%]": [round(rel_diff, 2)],
        }
    )
...
def create_energy_map(
    ...
    box_lon = [OWF_LON_MIN, OWF_LON_MAX, OWF_LON_MAX, OWF_LON_MIN, OWF_LON_MIN]
    box_lat = [OWF_LAT_MIN, OWF_LAT_MIN, OWF_LAT_MAX, OWF_LAT_MAX, OWF_LAT_MIN]
    ...
    plot1 = ax1.contourf(
        lon,
        lat,
        wrfswan_energy_ds["power_10m"].sum(dim="time") / 1000.0,
    ...
    plot2 = ax2.contourf(
        lon,
        lat,
        wrfstand_energy_ds["power_10m"].sum(dim="time") / 1000.0,
        transform=ccrs.PlateCarree(),
        cmap="Spectral_r",
        vmin=0,
        vmax=10500,
    ...
    for ax, title in zip([ax1, ax2, ax3], ["WRF-SWAN", "WRF", "(WRF-SWAN) - WRF"]):
        ...
        center_lon = (OWF_LON_MIN + OWF_LON_MAX) / 2.0
        center_lat = (OWF_LAT_MIN + OWF_LAT_MAX) / 2.0
...
def main() -> None:
    ...
    datasets = load_model_datasets(database_dir)
    ...
    df_wrfswan_wind = dataset_point_series(dset_wrfswan, "wspd", time_shift_hours=args.time_shift_hours)
    df_wrfstand_wind = dataset_point_series(dset_wrfstand, "wspd", time_shift_hours=args.time_shift_hours)
    df_wrfswan_power = dataset_point_series(dset_wrfswan_energy, "power_10m", time_shift_hours=0)
    df_wrfstand_power = dataset_point_series(dset_wrfstand_energy, "power_10m", time_shift_hours=0)

    validation_df = merge_validation_timeseries(df_wrfswan_wind, df_wrfstand_wind, df_lidar_hourly)
    ...
    energy_difference, relative_energy_difference = compute_energy_difference_fields(
        dset_wrfswan_energy, dset_wrfstand_energy, power_var="power_10m"
    )
    energy_summary = build_energy_summary(
        dset_wrfswan_energy, dset_wrfstand_energy, energy_difference, power_var="power_10m"
    )

    owf_wrfswan_energy = dset_wrfswan_energy.sel(
        y=slice(OWF_LAT_MIN, OWF_LAT_MAX), x=slice(OWF_LON_MIN, OWF_LON_MAX)
    )
    owf_wrfstand_energy = dset_wrfstand_energy.sel(
        y=slice(OWF_LAT_MIN, OWF_LAT_MAX), x=slice(OWF_LON_MIN, OWF_LON_MAX)
    )
    before_treatment = owf_wrfstand_energy["power_10m"].sum(dim="time").values.flatten()
    after_treatment = owf_wrfswan_energy["power_10m"].sum(dim="time").values.flatten()
    ...
    energy_map_created = create_energy_map(
        dset_wrfswan_energy, dset_wrfstand_energy, relative_energy_difference,
        figures_dir, args.save_figures,
    )
    ...
    print(
        f"\nOWF box: lat [{OWF_LAT_MIN}, {OWF_LAT_MAX}]  lon [{OWF_LON_MIN}, {OWF_LON_MAX}]"
    )
    print("\nEnergy summary (OWF box only):")
